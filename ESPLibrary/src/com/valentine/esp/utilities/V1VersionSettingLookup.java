...
public class V1VersionSettingLookup 
{
    ...
    /* return an array list of defaultCustomSweep */
    /* Added by Francky */

    public ArrayList<SweepDefinition> getV1DefaultCustomSweepsAsList() {

        int maxSweepIndex;
        Range [] customSweeps;

        ArrayList<SweepDefinition> rc = new ArrayList<SweepDefinition>();
        ...
        for ( int i = 0; i <= maxSweepIndex; i++ )
        {
            SweepDefinition swDef = new SweepDefinition();
            swDef.setIndex(i);
            swDef.setLowerFrequencyEdge(customSweeps[i].LoFreq);
            swDef.setUpperFrequencyEdge(customSweeps[i].HiFreq);
            rc.add(swDef);
        }
